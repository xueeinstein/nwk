  ...
  if (rollbacks.length) {
    chain(rollbacks.map(function (f) {
      ...
      if (er) {
        ...
        else if (noLog) rm("nwk-debug.log", reallyExit.bind(null, er));
        else writeLogFile(reallyExit.bind(this, er));
      } else {
        if (!noLog && code) writeLogFile(reallyExit);
        else rm("nwk-debug.log", reallyExit);
      }
    })
    rollbacks.length = 0;
  }
...
function errorHandler (er) {
  ...
  case "ENOGIT":
    ...
    log.error("", ["","Failed using git."
              ,"This is most likely not a problem with nwk itself."
  ...
  case "ENOPACKAGEJSON":
    log.error("package.json", [er.message
              ,"This is most likely not a problem with nwk itself."
              ,"nwk can't find a package.json file in your current directory."
              ].join("\n"))
    break
  ...
  case "ENOENT":
    log.error("enoent", [er.message
              ,"This is most likely not a problem with nwk itself"
              ,"and is related to nwk not being able to find a file."
              ,er.file?"\nCheck if the file '"+er.file+"' is present.":""
              ].join("\n"))
    break

  case "EGETNWHEADER":
    // TODO, design more details err message
    // e.g. "this nw version doesn't exist"
    log.error("egetnwheader", [er.message
              , "You can't get nw_headers now!"
              , "If you need help, you may report this error at:"
              , "     <https://github.com/xueeinstein/nwk/issues>"
              ].join("\n"))
    break
...
function writeLogFile (cb) {
  ...
  var fstr = writeStream("nwk-debug.log")
    , os = require("os")
    , out = "";
